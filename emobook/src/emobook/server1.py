# src/emobook/server.py
# Single-endpoint API (/insight) + Gradio UI mounted at /app
...
# --- Gradio JSON-Schema bool fix (prevents 500 on /app for some versions) ---
def _patch_gradio_client_bool_schema_bug():
    try:
        ...
        def _safe(schema, *args, **kwargs):
            # Guard boolean schemas (allowed by JSON-Schema, some gradio paths choke on them)
        ...
        # If anything fails here, just ignore—UI will likely still work.
        pass

_patch_gradio_client_bool_schema_bug()
# ---------------------------------------------------------------------------

# ----------------- helpers (reader-facing summaries) -----------------
...
async def insight(file: UploadFile = File(...), topk: int = Form(5)):
    """
    Upload a .txt, get a single LLM-written insight (string) + neighbors + meta.
...
# ----------------- Gradio UI mounted at /app -----------------
import gradio as gr
from gradio.routes import mount_gradio_app

def _gradio_insight(file_obj, topk):
    """Gradio callback: returns (markdown_explanation, markdown_neighbors, markdown_tags)."""
    if file_obj is None:
        return ("### Please upload a .txt file.", "", "")
    # robust read of uploaded file
    try:
        raw_bytes = file_obj.read()
    except Exception:
        # fallback to path if file-like fails
        raw_bytes = Path(getattr(file_obj, "name", "upload.txt")).read_bytes()
    fname = Path(getattr(file_obj, "name", "upload.txt")).name or "upload.txt"
...
def main():
    ...
    s1 = sub.add_parser("serve", help="Run HTTP API + Gradio UI")
